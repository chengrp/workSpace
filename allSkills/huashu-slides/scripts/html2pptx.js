...
async function addBackground(slideData, targetSlide, tmpDir, htmlDir = null) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    let imagePath = slideData.background.path;
    // Remove file:// prefix (file:/// has 3 slashes, we need to remove all)
    if (imagePath.startsWith('file://')) {
      imagePath = imagePath.replace(/^file:\/\/\//, '').replace(/^file\/\//, '');
    }
    // Convert relative path to absolute
    if (htmlDir && !path.isAbsolute(imagePath)) {
      imagePath = path.join(htmlDir, imagePath);
    }
...
function addElements(slideData, targetSlide, pres, htmlDir = null) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    // Throw all errors at once if any exist (unless ignoreValidation is true)
    if (validationErrors.length > 0) {
      if (ignoreValidation) {
        // Print warnings but continue
        console.warn(`[Validation Warnings for ${htmlFile}]:`);
    ...
    await addBackground(slideData, targetSlide, tmpDir, htmlDir);
