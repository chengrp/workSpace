...
const NANO_BANANA_REFS = resolve(PROJECT_DIR, 'toolkit', '.claude', 'skills', 'nano-banana-pro-prompts-recommend-skill', 'references');
// ---------------------------------------------------------------------------
// Size mapping
// ---------------------------------------------------------------------------
const SIZE_MAP = {
...
function resolveProvider(config, explicit) {
    ...
    if (defaultP && providers[defaultP]?.api_key)
        return [defaultP, providers[defaultP]];
...
// HTTP helper
// ---------------------------------------------------------------------------
async function httpRetry(url, init, retries = 3, timeoutMs = 120_000) {
    for (let i = 1; i <= retries; i++) {
        try {
            const resp = await fetch(url, { ...init, signal: AbortSignal.timeout(timeoutMs) });
            if (!resp.ok)
                throw new Error(`HTTP ${resp.status}: ${await resp.text().then(t => t.slice(0, 300))}`);
            return resp;
        }
        catch (e) {
            if (i === retries)
...
// Providers
// ---------------------------------------------------------------------------
async function generateGemini(prompt, apiKey, aspectRatio, model = 'imagen-3.0-generate-002') {
    ...
    if (!b64)
        throw new Error(`Gemini API 无返回: ${JSON.stringify(data).slice(0, 200)}`);
    return Buffer.from(b64, 'base64');
}
async function generateOpenAI(prompt, apiKey, size, model = 'gpt-image-1', baseUrl = 'https://api.openai.com/v1') {
    ...
    if (!items.length)
        throw new Error(`OpenAI API 无返回: ${JSON.stringify(data).slice(0, 200)}`);
    if (items[0].b64_json)
        return Buffer.from(items[0].b64_json, 'base64');
    if (items[0].url) {
...
async function generateDoubao(prompt, apiKey, size, model = 'doubao-seedream-5-0-260128', baseUrl = 'https://ark.cn-beijing.volces.com/api/v3') {
    const url = `${baseUrl}/images/generations`;
    const resp = await httpRetry(url, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    ...
    const items = (data.data ?? []);
    if (!items.length)
        throw new Error(`豆包 API 无返回: ${JSON.stringify(data).slice(0, 200)}`);
    if (items[0].b64_json)
        return Buffer.from(items[0].b64_json, 'base64');
 ...
 */
async function generateYouMind(prompt, apiKey, _size, _model, _baseUrl) {
...
function searchNanoBanana(keywords, maxResults = 3) {
    ...
    for (const cat of manifest.categories ?? []) {
        ...
        if (!existsSync(catFile))
            continue;
        try {
            const prompts = JSON.parse(readFileSync(catFile, 'utf-8'));
            for (const p of prompts) {
                if (!p?.sourceMedia?.length)
                    continue;
...
async function downloadNanaBananaImage(url, output) {
    try {
        const resp = await httpRetry(url, {}, 1, 30_000);
        const buf = Buffer.from(await resp.arrayBuffer());
...
// Fallback cover
// ---------------------------------------------------------------------------
function selectFallbackCover(color = '#3498db', mood = '') {
    const targetHue = COLOR_HUE_MAP[color.toLowerCase()] ?? 'blue';
    const candidates = [];
    for (const [, meta] of Object.entries(COVER_PALETTE)) {
        let score = 0;
        if (meta.hue === targetHue)
            score += 3;
        if (mood && meta.mood === mood)
            score += 2;
        if (meta.tone === (['orange', 'warm'].includes(targetHue) ? 'warm' : 'cool'))
            score += 1;
...
async function downloadFallbackCover(url, output) {
    try {
...
async function main() {
    ...
    try {
        const imageBytes = await genFn(args.prompt, providerCfg.api_key, sizeVal, providerCfg.model, providerCfg.base_url);
...
export { generateGemini, generateOpenAI, generateDoubao, searchNanoBanana, selectFallbackCover, downloadFallbackCover, resolveProvider, GENERATORS, SIZE_MAP, };
