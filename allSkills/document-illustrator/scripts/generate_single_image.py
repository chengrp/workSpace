...
def generate_image(title, content, style_prompt, output_path, aspect_ratio="16:9", resolution="2K", is_cover=False):
    ...
    # 获取 API 密钥（支持多种环境变量名）
    api_key = os.environ.get("GEMINI_API_KEY") or os.environ.get("ALL_IMAGE_GOOGLE_API_KEY") or os.environ.get("GOOGLE_API_KEY")
    if not api_key:
        print("错误: 未设置 API 密钥环境变量", file=sys.stderr)
        print("请在 .env 文件中设置以下任一环境变量:", file=sys.stderr)
        print("  - GEMINI_API_KEY", file=sys.stderr)
        print("  - ALL_IMAGE_GOOGLE_API_KEY", file=sys.stderr)
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                response_modalities=['IMAGE', 'TEXT'],  # 修复：添加 TEXT 模态
                image_config=types.ImageConfig(
                    aspect_ratio=aspect_ratio
                    # 修复：移除 image_size 参数（不支持）
                )
            )
        )
