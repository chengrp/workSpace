...
const DEFAULT_SETTINGS = {
    ...
    aiImageAutoUpload: false,
    aiImagePatterns: ["ai-", "generated", "claude", "assistant"],
    reuploadNetworkImages: true,
};
class SettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            .setName("AI图片自动上传")
            .setDesc("启用后，当AI生成图片并插入到文档中时自动上传到PicGo服务器")
            .addToggle(toggle => toggle
            .setValue(this.plugin.settings.aiImageAutoUpload)
            .onChange(async (value) => {
            this.plugin.settings.aiImageAutoUpload = value;
            await this.plugin.saveSettings();
        }));
        if (this.plugin.settings.aiImageAutoUpload) {
            new obsidian.Setting(containerEl)
                .setName("AI图片识别模式")
                .setDesc("文件名包含以下模式将被识别为AI生成的图片（用逗号分隔）")
                .addText(text => text
                .setPlaceholder("ai-, generated, claude, assistant")
                .setValue(this.plugin.settings.aiImagePatterns.join(', '))
                .onChange(async (value) => {
                    this.plugin.settings.aiImagePatterns = value.split(',').map(s => s.trim()).filter(s => s);
                    await this.plugin.saveSettings();
                }));
        }
    }
...
class imageAutoUploadPlugin extends obsidian.Plugin {
    ...
    setupAIImageHandler() {
        if (!this.settings.aiImageAutoUpload) {
            return;
        }
        ...
        this.registerEvent(this.app.workspace.on("editor-change", (editor, view) => {
            ...
            if (view instanceof obsidian.MarkdownView) {
                ...
                const lastContent = this.lastEditorContent || "";
                ...
                if (content !== lastContent) {
                    ...
                    // 查找新插入的图片
                    const existingImages = this.findExistingImages(lastContent);
                    const currentImages = this.findExistingImages(content);

                    // 找到新增的图片
                    const newImages = currentImages.filter(img => !existingImages.some(e => e.path === img.path));

                    // 处理新增的AI图片
                    this.processAIImages(editor, newImages);
                }
            }
        }));
    }
    ...
    findExistingImages(content) {
        ...
        while ((match = imageRegex.exec(content)) !== null) {
            images.push({
                ...
                path: match[2],
                fullMatch: match[0],
                isDataUrl: match[2].startsWith('data:image/')
            });
        }
    ...
    // 处理AI图片
    async processAIImages(editor, images) {
        if (images.length === 0) return;

        for (const image of images) {
            // 检查是否是AI生成的图片
            if (this.isAIImage(image.path, image)) {
                try {
                    let uploadPath = image.path;
                    let isTempFile = false;

                    // 如果是base64数据URL，需要保存为临时文件
                    if (image.isDataUrl) {
                        uploadPath = await this.saveDataUrlToFile(image.path, image.alt);
                        isTempFile = true;
                    }
                    // 如果是网络图片，需要下载到临时文件
                    else if (image.path.startsWith('http://') || image.path.startsWith('https://')) {
                        uploadPath = await this.downloadImageToFile(image.path, image.alt);
                        isTempFile = true;
                    }

                    // 上传图片
                    const uploadResult = await this.uploader.uploadFiles([uploadPath]);

                    if (uploadResult.success) {
                        const url = uploadResult.result[0];

                        // 替换为上传后的链接
                        const newMarkdown = `![${image.alt}](${url})`;
                        const editorContent = editor.getValue();
                        const updatedContent = editorContent.replace(image.fullMatch, newMarkdown);

                        editor.setValue(updatedContent);

                        // 通知用户
                        new obsidian.Notice(`AI图片已上传到PicGo服务器并替换为外链`);
    ...
    // 下载网络图片到临时文件
    async downloadImageToFile(url, alt) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const fileName = `ai-network-${timestamp}.jpg`;
            const tempDir = this.app.vault.adapter.basePath;
    ...
    // 将base64数据URL保存为临时文件
    async saveDataUrlToFile(dataUrl, alt) {
        const fs = require('fs');
        const path = require('path');
        ...
        // 生成文件名
        const timestamp = Date.now();
        const fileName = `ai-generated-${timestamp}.${imageType === 'jpeg' ? 'jpg' : imageType}`;
        const tempDir = this.app.vault.adapter.basePath;
        const filePath = path.join(tempDir, fileName);
    ...
    // 判断是否是AI生成的图片
    isAIImage(path, imageData) {
        // 如果是网络图片，认为是AI生成的
        if (path.startsWith('http://') || path.startsWith('https://')) {
            return true;
        }

        // 如果是base64数据URL，认为是AI生成的
        ...
        return this.settings.aiImagePatterns.some(pattern =>
